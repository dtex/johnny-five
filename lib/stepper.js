 ...
 *    enable: number  // (optional enable pin)
 *  }
 * });
 *
 *
...
function Stepper(opts) {
  ...
  if (Array.isArray(this.pins)) {
    ...
    if (pinArray.length === 2 || pinArray.length === 3) {
      // Using an array of 2 or 3 pins requres a TYPE
...
Stepper.prototype.step = function(stepsOrOpts, callback) {
  ...
  if (isValidStep) {
    ...
    params.push(function(complete) {
      ...
      if (this.pins.enable) {
        this.io.digitalWrite(this.pins.enable, 0);
      }
      callback(null, complete);
    }.bind(this));

    if (this.pins.enable) {
      this.io.digitalWrite(this.pins.enable, 1);
    }
